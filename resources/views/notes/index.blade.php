<x-app-layout :title="'All Notes ' . ' - ' . config('app.name')">
    <x-slot name="header">
                ...
                <div class="relative mx-4 mt-4 overflow-hidden text-gray-700 bg-white rounded-none bg-clip-border">
                    ...
                    <div class="flex items-center justify-between gap-8 mb-8">
                        <div>
                            ...
                            <p
                                ...
                                {{  request('completed') ? 'completed' : (request('completed') === '0' ? 'in progress' : 'all') }}
                                notes
                            </p>
                        </div>
                        <div class="flex flex-col gap-2 shrink-0 sm:flex-row">
                            ...
                            <a href="{{ route('notes.create') }}"
                                ...
                                type="button">
                                <svg viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" stroke-width="2"
                    ...
                    <div class="flex flex-col items-center justify-between gap-4 md:flex-row">
                        <div class="block w-full overflow-hidden md:w-max">
                            <nav>
                                <ul role="tablist" class="relative flex flex-row space-x-2 py-2 px-1.5 rounded-lg ">
                                    {{-- Фільтр "All" --}}
                                    <x-tab :href="route('notes.index')" :active="request('completed') === null">
                                        All
                                    </x-tab>
                                    {{-- Фільтр "Completed" --}}
                                    <x-tab :href="route('notes.index', ['completed' => 1])"
                                        :active="request('completed') === '1'">
                                        Completed
                                    </x-tab>
                                    {{-- Фільтр "InProgress" --}}
                                    <x-tab :href="route('notes.index', ['completed' => 0])"
                                        :active="request('completed') === '0'">
                                        In Progress
                                    </x-tab>
                                </ul>
                            </nav>
                                    ...
                                    <div class="w-max">
                                        <div
                                            ...
                                            @if ($note->resourceType)
                                                <a href="{{ route('notes.index', ['resource_type_id' => $note->resourceType->id]) }}"
                                                    class="">{{ $note->resourceType->name }}</a>
                                            @else
